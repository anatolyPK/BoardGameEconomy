        ...
        async with self._session() as session:
            for game_info in games_info:
                ...
                if not existing_game.first():
                    ...
                    await session.flush()
                    game_id = instance.id
                    await game_search_repository.create_game_name_ru(
                        session, game_info.name_ru, game_id
                    )
    ...
    async def search_games(self, game_name: str) -> GameSearchSchema:
        async with self._session() as session:
            ...
            stmt = (
                select(self.model)
                .where(ilike_expr)
                .options(selectinload(self.model.base_game))
            )
...
game_downloader_repository = GameRepository(
    model=Game, db_session=db_helper.get_db_session_context
)

game_search_repository = GameNameRuRepository(
    model=GameRuName, db_session=db_helper.get_db_session_context
)
