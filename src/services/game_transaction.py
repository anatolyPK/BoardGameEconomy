...
from schemas.transactions import (
    ...
    GameAddTransactionWithUserSchema,
    GameTransactionPatchSchema,
    GameTransactionPatchTransactionWithUserSchema,
    GameTransactionsSchema,
    GameTransactionSelectedGameSchema,
)
...
class GameTransactionService(BaseService):
    ...
    async def add_game(self, game: GameAddTransactionSchema, user: User) -> ModelType:
        game_with_user_id = GameAddTransactionWithUserSchema(
            **game.dict(), user_id=user.id
        )
        added_game = await self.repository.create(game_with_user_id)
        return added_game

    async def update_transaction(
        self, transaction: GameTransactionPatchSchema, user: User, pk: int
    ) -> ModelType:
        transaction_with_user_id = GameTransactionPatchTransactionWithUserSchema(
            **transaction.dict(), user_id=user.id
        )
        added_transaction = await self.repository.update(
            transaction_with_user_id, pk=pk
        )
    ...
    async def get_users_board_games_transactions(
        self, user: User
    ) -> GameTransactionsSchema:
        result_orm = await self.repository.get_user_transactions(user_id=user.id)
        result_dto = [
            GameTransactionSelectedGameSchema.model_validate(game, from_attributes=True)
