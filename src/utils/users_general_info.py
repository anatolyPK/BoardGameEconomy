from models.base import GameTransaction, User
...
def get_users_games_general_info(games_schema: GameTransactionsSchema, user: User):
    ...
    amount_of_games_in_stock = sum(game.purchase_price for game in games if not game.selling_price)
    amount_of_selling_games = sum(game.selling_price - game.purchase_price for game in games if game.selling_price)
    total_amount = amount_of_games_in_stock + amount_of_selling_games
    average_price_of_game_in_stock = amount_of_games_in_stock / game_in_stock if game_in_stock else 0
